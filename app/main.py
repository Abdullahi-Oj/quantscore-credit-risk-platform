...
def interpret_result(probability: float) -> dict:
    if probability < 0.35:
        return {"risk_category": "APPROVE", "confidence": "High", "reason": "Low predicted default risk based on income stability and repayment behavior."}
    elif probability < 0.65:
        return {"risk_category": "REVIEW", "confidence": "Medium", "reason": "Moderate risk detected. Manual review recommended."}
    else:
        return {"risk_category": "REJECT", "confidence": "High", "reason": "High default risk driven by income, repayment ratio, and credit history."}
...
def predict(request: Request, data: CreditData, api_key: str = Depends(verify_api_key)):
    try:
...
def explain(request: Request, data: CreditData, api_key: str = Depends(verify_api_key)):
    try:
        df = pd.DataFrame([transform_input(data.dict())])
        df = engineer_features(df)
        X_scaled = models.scaler.transform(df[models.features])
...
def batch_predict(request: Request, data: List[CreditData], api_key: str = Depends(verify_api_key)):
    try:
...
@app.get("/health")
def health():
    return {"status": "ok", "model_loaded": True, "model_version": models.version()}
